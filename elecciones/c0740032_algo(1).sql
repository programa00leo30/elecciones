...
CREATE ALGORITHM=UNDEFINED DEFINER=`c0740032`@`%` SQL SECURITY DEFINER VIEW `SubEnLista` AS select `d`.`idCargo` AS `idCargo`,`c`.`idLista` AS `idLista`,`d`.`id` AS `idCandidato`,concat(`d`.`nombre`,', ',`d`.`apellido`) AS `candiato` from (`candidatosEnLista` `c` left join `candidatos` `d` on((`d`.`id` = `c`.`idCandidato`))) group by `c`.`idLista`,`d`.`idCargo`;
